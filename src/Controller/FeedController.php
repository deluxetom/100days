                ...
                if (count($series) > 0) {
                    ...
                    for ($s = 0; $s < count($series); $s++) {
                        if ($s == 0 && $serie['timestamp'] > $series[$s]['timestamp']) {
                            $tmp[] = $serie;
                            $tmp[] = $series[$s];
                        } else if ((isset($series[$s + 1]) && $serie['timestamp'] <= $series[$s]['timestamp'] && $serie['timestamp'] > $series[$s + 1]['timestamp']) || (!isset($series[$s + 1]) && $serie['timestamp'] <= $series[$s]['timestamp'])) {
                            $tmp[] = $series[$s];
                            $tmp[] = $serie;
                        } else {
                            $tmp[] = $series[$s];
